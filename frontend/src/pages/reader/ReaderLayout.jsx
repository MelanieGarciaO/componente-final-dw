...
import { BookOpen, LibraryBig, User, LogOut, Menu, X } from 'lucide-react'
import { useAuth } from '../../context/AuthContext'

const navItems = [
  { to: '/reader/catalog', label: 'Catálogo', icon: LibraryBig },
  { to: '/reader/loans', label: 'Mis Préstamos', icon: BookOpen },
  { to: '/reader/profile', label: 'Perfil', icon: User },
]
      ...
      >
        ...
        <div className="px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: 'rgba(201,162,39,0.8)' }}>Lector</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-3 pb-6">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-white/50 hover:bg-red-500/10 hover:text-red-300"
          >
            <LogOut size={18} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex-shrink-0 flex items-center justify-between px-6 py-4 bg-white border-b border-border" style={{ boxShadow: '0 1px 8px rgba(31,42,60,0.06)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium text-gray-400">Portal del lector</p>
              <h1 className="text-lg font-bold text-navy">Biblioteca</h1>
            </div>
          </div>
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white bg-navy">{initials}</div>
        </header>
